...
| [templates/build-deploy-bundles.py](templates/build-deploy-bundles.py) | Build script that resolves `@`-include directives into self-contained agent bundles. Copied into output libraries during Phase 4. Supports `--all-inclusive` for runtimes where conditional fetch is unreliable. |
